...
export const SpinningBorderButton: React.FC<SpinningBorderButtonProps> = ({
  ...
  return (
    <button
      className={cn(
        "duration-200 relative inline-flex h-10 overflow-hidden rounded-full p-[1px]",
        className
      )}
      {...props}
    >
      ...
      <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-slate-950 px-3 py-1 font-medium font-bmono text-sm text-yellow-500 backdrop-blur-3xl">
        {children}
      </span>
    </button>
  );
